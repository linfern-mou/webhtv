...
- 已完成：播放结束或退出 K歌时显示结果弹窗，包含娱乐分、演唱覆盖和最长连击，零分也显示。
...
- 已完成：结束或退出 K歌时显示结果弹窗，零分也显示，并补充演唱覆盖和最长连击。
- 已完成：普通音符、金色音符、自由音符、Rap 音符的基础权重和评分处理。
- 已完成：文件选择器 / URL 导入 UltraStar `.txt` 并缓存到歌曲 key。
- 已完成：MIDI / KAR 主旋律轨导入启发式。
- 已完成：实时状态浮层显示当前音高、命中状态、目标音偏差、麦克风音量和连续命中/连续发声提示。
- 待完善：更完整的横向目标音符条、用户音高曲线、逐句得分、评级、金色音符视觉特效。
...
5. 已完成基础版：有谱评分 UI，包括当前音高、目标音偏差、命中状态、音量和连击提示。
6. 下一步：更完整的目标音符条、用户音高线、逐句得分和评级。
7. 后续：TV K歌布局和手机当麦克风。
