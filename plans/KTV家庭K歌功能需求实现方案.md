...
- 已完成：实时 UI 显示得分、当前音高、命中/偏差、目标音/当前音偏差条、目标音符时间线、麦克风音量组件。
...
- 已完成：实时状态浮层显示当前音高、命中状态、目标音偏差、目标音符时间线、麦克风音量、分数/覆盖条和连续命中/连续发声提示。
- 已完成：按分数给出 S+ / S / A / B / C / D / E 娱乐评级。
- 待完善：更完整的大屏目标音符条、用户音高历史曲线、逐句得分、金色音符视觉特效。
...
5. 已完成基础版：有谱评分 UI，包括当前音高、目标音偏差、命中状态、目标音符时间线、音量、分数/覆盖条、连击提示和评级。
6. 下一步：更完整的大屏目标音符条、用户音高历史曲线和逐句得分。
7. 后续：TV K歌布局和手机当麦克风。
